...
export default function SampleProductSlider({ spslides }) {
      ...
      spaceBetween={30}
       slidesPerView={3}
        grid={{
         ...
         breakpoints={{
        320:  { slidesPerView: 1, grid: { rows: 1 } }, // Tablet
        520:  { slidesPerView: 2, grid: { rows: 1 } }, // Tablet
        991: { slidesPerView: 3, grid: { rows: 2 } }, // Desktop
      }}
        onSwiper={setSwiperInstance}
    >
      {spslides.map((slide, idx) => (
        <SwiperSlide key={idx}>
          <ul>{slide.type === 'image' ? (
            <li className="sws_img"><img src={slide.src} alt={slide.alt} style={{ width: '100%' }} /></li>
          ) : (
